...
let mUrl = null;
let expire = 4102329600; // 2099-12-31
let password = '';
let proxyIP = '';
let bRunSave = true;

// متغیرهای کشینگ
let cachedExpireTimestamp = null;
let cachedPassword = null;
let cachedConfigName = null;
let cachedUsedVolume = null;
let cachedTotalVolume = null;
let lastFetchTime = 0; // زمان آخرین دریافت از KV
const CACHE_DURATION = 600000; // ۱۰ دقیقه به میلی‌ثانیه
...
export default {
    async fetch(request, env, ctx) {
        ...
        try {
			...
			//----------
			const k1 = atob('Y2xhc2g=');
            const k2 = atob('bWlob21v');
            const k3 = atob('c3ViY29udmVydGVy');

            if (userAgent.includes(k1) || (userAgent.includes(k2) && !userAgent.includes(k3))) {
                try {
                    const reqUrl = new URL(request.url);
                    const bU = atob('aHR0cHM6Ly9zdWIucHRxeS5zZXJ2MDAubmV0L3N1Yg==');
                    const cU = atob('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL2NtbGl1L0FDTDRTU1IvbWFpbi9DbGFzaC9jb25maWcvQUNMNFNTUl9PbmxpbmVfRnVsbF9NdWx0aU1vZGUuaW5p');
                    
                    const fU = `${bU}?target=${k1}&url=${encodeURIComponent(reqUrl.href)}&insert=false&config=${encodeURIComponent(cU)}&emoji=true&list=false&tfo=false&scv=true&fdn=false&sort=false&new_name=true`;
                    
                    const sRes = await fetch(fU, {
            ...
            mUrl = `https://${url.host}/`
                if (bRunSave) {
                    const result = await saveToPool(mUrl, "");
                }
                // =====================================================
                password = await getCachedPassword(env);
            ...
            if (env.CFPORTS)
                httpsPorts = await ADD(env.CFPORTS);
            if (env.BAN)
                banHosts = await ADD(env.BAN);
            ...
            if (!upgradeHeader || upgradeHeader !== "websocket") {
                ...
                case `/${password}/config.json`:
                    if (url.searchParams.get('token') === await MD5MD5(fakeUserID + UA))
                        return await config_Json(password, request.headers.get('Host'), sub, UA, 请求CF反代IP, url, fakeUserID, fakeHostName, env);
                ...
                case `/${password}`:
                    ...
                    expire = await getCachedExpireTimestamp(env);
                ...
                default:
                    if (env.URL302)
                        ...
                        return new Response('不用怀疑！你PASSWORD就是错的！！！', {
                            status: 404
                        });
                }
            } else {
                ...
                const expireDate = await getCachedExpireTimestamp(env); // ✅ از تابع جدید استفاده کن

                // استخراج دقیقه فعلی

                if (currentTimestamp > expireDate) {
                    return new Response('❌ اشتراک شما منقضی شده است!\n\nلطفاً برای تمدید به کانال @TutiVPN مراجعه کنید.', {
                        status: 403,
                        headers: {
                            'Conteent-Type': 'text/plain; charset=utf-8',
                        }
                    });
                }
...
async function 特洛伊OverWSHandler(request) {
    ...
    readableWebSocketStream.pipeTo(new WritableStream({
            async write(chunk, controller) {
				...
				const result = parseMELOHeader(chunk);
				
                const { hasError, message, portRemote, addressRemote, rawClientData, addressType, version } = result;
                ...
                if (!banHosts.includes(addressRemote)) {
                    ...
                    handleTCPOutBound(remoteSocketWapper, addressRemote, portRemote, rawClientData, webSocket, log, addressType, version);
                } else {
...
async function parse特洛伊Header(buffer) {
    ...
    case 1:
        ...
        address = new Uint8Array(
                socks5DataBuffer.slice(addressIndex, addressIndex + addressLength)).join(".");
        break;
    case 3:
        addressLength = new Uint8Array(
                socks5DataBuffer.slice(addressIndex, addressIndex + 1))[0];
        addressIndex += 1;
...
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, log, addressType, meloVersion) {
        ...
        if (启用SOCKS5全局反代) {
            ...
            try {
                ...
                const timer = setTimeout(() => { 
                    timeout = true; 
                    try { tcpSocket2.close(); } catch(e) {} 
                }, 5000);
                
                await tcpSocket2.opened;
                clearTimeout(timer);
                if (timeout) throw new Error('Socket Hang Timeout');
	...
	async function retry() {
        ...
        tcpSocket2.closed.catch((error) => {}).finally(() => {
    ...
    const meloResponseHeader = new Uint8Array([meloVersion[0], 0]);
    remoteSocketToWS(tcpSocket, webSocket, retry, log, meloResponseHeader);
...
async function remoteSocketToWS(remoteSocket, webSocket, retry, log, meloResponseHeader = null) {
    let hasIncomingData = false;
    let meloHeader = meloResponseHeader;
    ...
    try {
        ...
        while (true) {
            ...
            while (offset < value.byteLength) {
                const currentChunk = value.slice(offset, offset + MAX_CHUNK_SIZE);
                offset += MAX_CHUNK_SIZE;
                
                if (meloHeader) {
                    webSocket.send(await new Blob([meloHeader, currentChunk]).arrayBuffer());
                    meloHeader = null;
                } else {
                    webSocket.send(currentChunk);
                }
        ...
        safeCloseWebSocket(webSocket);
        // اگر اتصالی برقرار نشد، فرآیند Retry فعال می‌شود
        if (!hasIncomingData && retry) {
            if (typeof log === 'function') log(`retry`);
            retry();
        }
    }
}
...
async function get特洛伊Config(password, hostName, sub, UA, 请求CF反代IP, _url, fakeUserID, fakeHostName, env) {
    if (sub) {
        ...
        if ((addresses.length + addressesapi.length + addressescsv.length) == 0) {
            ...
            if (cachedConfigName !== null) {
                cachedConfigName = persianToEnglishDigits(cachedConfigName);
                addresses = addresses.concat(
                        cfips.map(cidr => generateRandomIPFromCIDR(cidr) + ':' + randomPorts[Math.floor(Math.random() * randomPorts.length)] + '#🇧🇪 [' + cachedConfigName + '] - S' + String(counter++).padStart(2, '0')));
            } else {
                addresses = addresses.concat(
                        cfips.map(cidr => generateRandomIPFromCIDR(cidr) + ':' + randomPorts[Math.floor(Math.random() * randomPorts.length)] + '#🇧🇪 - S' + String(counter++).padStart(2, '0')));
            }
...
function subAddresses(host, pw, userAgent, newAddressesapi, newAddressescsv) {
    ...
    const addressPool = [...uniqueAddresses];
    const targetCount = Math.min(5, addressPool.length);
    for (let i = 0; i < targetCount; i++) {
        const randomIndex = Math.floor(Math.random() * addressPool.length);
        randomAddresses.push(addressPool.splice(randomIndex, 1)[0]);
    }

    const parsedNodes = randomAddresses.map(address => {
        let port = "-1";
        ...
        if (port == "-1") port = "443";
        let 最终路径 = path;
        const matchingProxyIP = proxyIPPool.find(p => p.includes(address));
        if (matchingProxyIP) 最终路径 = `/proxyip=${matchingProxyIP}`;
        return { address, port, addressid, 最终路径 };
    });

    let 密码 = pw;
    if (!userAgent.includes('subconverter')) 密码 = encodeURIComponent(pw);
    
    const 附加协议 = atob('dmxlc3M=');

    const meloLinks = parsedNodes.map((node, index) => {
        const cleanRemark = node.addressid.split(/ - [STV]/)[0];
        const currentRemark = cleanRemark + ' - V' + (index + 1);
        const fragmentParam = index < 3 ? `&fragment=${encodeURIComponent('100-200,10-20,tlshello')}` : '';
        
        return `${附加协议}://${密码}@${node.address}:${node.port}?encryption=none&security=tls&sni=${host}&alpn=http%2F1.1&fp=chrome&type=ws&host=${host}&path=${encodeURIComponent(node.最终路径) + allowInsecure}&packetEncoding=xudp${fragmentParam}#${encodeURIComponent(currentRemark)}`;
    });

    const responseBody = meloLinks.join('\n');
...
//-------
async function fetchSubscription(env, selectFields) {
    const supabaseUrl = "https://rgkcynzubjlyfvucgdrj.supabase.co";
    const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJna2N5bnp1YmpseWZ2dWNnZHJqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzA3NDgyMjIsImV4cCI6MjA4NjMyNDIyMn0.0I-N9htgP2ZoQRJ7wXDYxqBcK1zVgAa6RqukbD5Bu-M";
    const now = new Date().toISOString();
    const url = mUrl;
    const params = new URLSearchParams();
    params.append('select', selectFields + ',pool!inner(url)');
    params.append('pool.url', `eq.${url}`);
    params.append('status', 'eq.true');
    params.append('ended_at', `gt.${now}`);

    try {
        const response = await fetch(`${supabaseUrl}/rest/v1/subscriptions?${params.toString()}`, {
            method: 'GET',
            headers: {
                'apikey': supabaseKey,
...
async function getCachedPassword(env) {
    const now = Date.now();
    // اگر کش وجود دارد و هنوز منقضی نشده، از کش استفاده کن
    if (cachedPassword !== null && (now - lastFetchTime) < CACHE_DURATION) {
        return cachedPassword;
    }

    try {

        const sub = await fetchSubscription(env, 'username,ended_at,config_name,total_volume,used_volume');

        if (sub) {
            // فقط نام کاربری را برمی‌گرداند
            cachedPassword = sub.username;
            // تبدیل تاریخ به تایم‌استمپ (میلی‌ثانیه)
            const timestamp = Math.floor(new Date(sub.ended_at).getTime() / 1000);
            cachedExpireTimestamp = timestamp;
            cachedConfigName = sub.config_name;
            cachedTotalVolume = sub.total_volume;
            cachedUsedVolume = sub.used_volume;
        } else {
            cachedPassword = "status: 404"
        }
        // زمان دریافت را آپدیت کن
        lastFetchTime = now;
        return cachedPassword;
    } catch (e) {
        return "status: 404";
...
// هندلر 2: دریافت فقط تاریخ انقضا (Text/Number)
async function getCachedExpireTimestamp(env) {
    const now = Date.now();
    // اگر کش وجود دارد و هنوز منقضی نشده، از کش استفاده کن
    if (cachedExpireTimestamp !== null && (now - lastFetchTime) < CACHE_DURATION) {
        return cachedExpireTimestamp;
    }

    try {

        const sub = await fetchSubscription(env, 'username,ended_at');
        if (sub) {
            // فقط نام کاربری را برمی‌گرداند
            cachedPassword = sub.username;
            // تبدیل تاریخ به تایم‌استمپ (میلی‌ثانیه)
            ...
            cachedExpireTimestamp = 0; // مقدار پیش‌فرض
        }
        ...
        console.error('Error fetching expire timestamp:', e);
        return expire; // در صورت خطا، مقدار پیش‌فرض برگردان
    }
}
...
// دریافت تاریخ فعلی از KV

// به‌روزرسانی تاریخ

// تابع اعتبارسنجی تاریخ

// دریافت تاریخ ذخیره شده به فرمت Unix Timestamp
async function getDateByUnixTimestamp(env) {
    try {
        const storedDate = await env.DATE_STORAGE.get('current_date');

        if (!storedDate) {
            // اگر تاریخ ذخیره نشده، از مقدار expire استفاده کن
            return expire;
        }

        // تبدیل تاریخ به timestamp
        const dateObj = new Date(storedDate);
        const timestamp = Math.floor(dateObj.getTime() / 1000); // به ثانیه

        return timestamp;
    } catch (error) {
        console.error('Error getting date by timestamp:', error);
        return expire; // در صورت خطا، مقدار پیش‌فرض برگردانده شود
    }
}
...
async function config_Json(userID, hostName, sub, UA, 请求CF反代IP, _url, fakeUserID, fakeHostName, env) {
    ...
    let CF访问方法 = "auto";
    if (enableSocks)
        CF访问方法 = enableHttp ? "http" : "socks5";
    else if (proxyIP && proxyIP != '')
        CF访问方法 = "proxyip";
    else if (请求CF反代IP == 'true')
        CF访问方法 = "auto";
...
function config_Html(token = "test", proxyhost = "") {
    let bTime = ' ';
    let bHajm = ' ';
    if (cachedExpireTimestamp !== null && cachedExpireTimestamp > 0) {
        bTime = 'تاریخ پایان اشتراک: ' + timestampToShamsi(cachedExpireTimestamp);
    }

    if (cachedTotalVolume !== null && cachedTotalVolume > 0) {
        bHajm = 'حجم کل: ' + cachedTotalVolume + ' گیگابایت | حجم مصرفی: ' + cachedUsedVolume + ' گیگابایت';
    }
    ...
    const html = `
				...
				<title id="pageTitle">Configuration Page</title>
				...
				* {
				...
				<a href="https://t.me/TutiVpn" target="_blank" class="social-link" title="TutiVPN Channel"   >
				...
				<a href="https://t.me/TutiVpn_bot" target="_blank" class="social-link" title="TutiVPN Bot" style="display: none;" >
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
				<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 14.5v-9l6 4.5-6 4.5z"/>
				</svg>
				</a>
				</div>
				<h1 id="pageHeader">VPN</h1>
				</div>
				...
				<button class="advanced-settings-btn" onclick="openAdvancedSettings()" style="display: none;">⚙️ Custom Subscription Settings</button>
				</div>
				...
				<div class="section details-section" style="display: none;">
				<details>
				<summary>
				<div class="summary-content" >
				<div class="summary-title" >📡 Best IP Subscription Configuration</div>
				<div class="summary-subtitle" id="kvStatus"></div>
				</div>
				<div class="summary-actions">
				<a id="bestipBtn" class="summary-btn disabled">⚡ Online Best IP Selection</a>
				<a id="editBtn" class="summary-btn disabled">📝 Edit Best IP List</a>
				</div>
				</summary>
				...
				<details>
				<summary>🔄 Subscription Converter Configuration</summary>
				<div class="details-content">
				...
				<details>
				<summary>⚙️ Detailed Configuration Info</summary>
				<div class="details-content">
				...
				<p><strong>1. PassWall/PassWall2:</strong> Set <strong>User-Agent</strong> to <strong>PassWall</strong> in subscription settings.</p>
				<p><strong>2. SSR+ Routing Plugin:</strong> Recommended to use <strong>Base64 Subscription URL</strong> for subscription.</p>
				</div>
				</details>
				</div>
				</div>
				...
				<h3>⚙️ Custom Subscription Settings</h3>
				...
				<button class="modal-btn modal-btn-secondary" onclick="closeAdvancedSettings()">Back</button>
				...
				updatePageTitles();
				updateKVStatus();
				// Display User-Agent in footer
				//document.getElementById('userAgent').textContent = 'User-Agent: ' + configData.UA;
				document.getElementById('userAgent').textContent = '${bTime}';
				document.getElementById('userAgent2').textContent = '${bHajm}';
				} catch (error) {
				console.error('Failed to load configuration:', error);
				document.getElementById('loading').innerHTML = '<p style="color: red;">❌ Failed to load configuration, please refresh the page</p>';
				}
				}
				...
				{ name: 'Surge Subscription', suffix: '?surge', primary: false }
				];
				container.innerHTML = '';
				// Create primary subscription (Adaptive Subscription)
				...
				// Create additional subscriptions container
				...
				showMoreBtn.textContent = '📋 Hide Subscription Formats';
				}
				}
				...
				// Create a config-grid container for vertical layout
				...
				clashCard.innerHTML =
				'<div class="link-label">Clash Config Snippet</div>' +
				...
				{ label: 'Skip TLS Verification', value: config.SCV === 'true' ? '✅ Enabled' : '❌ Disabled' }
				];
				...
				items.push({ label: 'CloudflareCDN Access Mode', value: 'Auto' });
				} else {
				...
				if (cf2cdn === 'proxyip') {
				items.push({ label: 'CloudflareCDN Access Mode', value: 'ProxyIP' });
				...
				if (isGlobal) {
				items.push({ label: 'CloudflareCDN Access Mode', value: 'Global SOCKS5' });
				} else {
				items.push({ label: 'CloudflareCDN Access Mode', value: 'SOCKS5' });
				...
				if (isGlobal) {
				items.push({ label: 'CloudflareCDN Access Mode', value: 'Global HTTP' });
				} else {
				items.push({ label: 'CloudflareCDN Access Mode', value: 'HTTP' });
				...
				// Other cases, display original GO2CF value
				items.push({ label: 'CloudflareCDN Access Mode', value: proxy.GO2CF });
				}
				}
				let html = '';
				items.forEach(item => {
				...
				{ label: 'Best IP Subscription Generator', value: sub.SUB },
				{ label: 'ADDCSV Speed Threshold', value: sub.DLS }
				];
				// Only display these configs when SUB is "local"
				if (sub.SUB === 'local') {
				subItems.push(
				{ label: 'ADD (TLS Best IP)', value: sub.ADD.join('<br>') },
				//{ label: 'ADDNOTLS (Non-TLS Best IP)', value: sub.ADDNOTLS.join('<br>') },
				...
				const sub = configData.sub;
				let items = [];
				// Check subscription converter backend status
				const backendUrl = sub.SUBAPI;
				const backendStatus = await checkBackendStatus(backendUrl);
				items.push({
				label: 'Subscription Converter Backend',
				value: backendStatus.display
				});
				items.push({
				label: 'Subscription Converter Config',
				value: sub.SUBCONFIG
				});
				let html = '';
				items.forEach(item => {
				...
				return {
				status: 'failed',
				display: backendUrl + ' ❌ Subscription converter backend unavailable'
				};
				}
				function updateActionButtons() {
				const editBtn = document.getElementById('editBtn');
				const bestipBtn = document.getElementById('bestipBtn');
				// Enable buttons only when KV is true and SUB is "local"
				...
				document.getElementById('pageHeader').textContent = '${bTime}';			
				}
				}
				...
				kvStatus.textContent = 'KV Namespace 🔴 Not Bound';
				}
				}
				...
				// Check if it's an important message (contains specific keywords)
				const isImportant = message.includes('re-copy') || message.includes('custom settings');
				if (isImportant) {
				// Important toast style - more prominent
				...
				// Remove protocol prefix and trailing slash
				let formatted = input.trim()
				.replace(/^socks5?:\\/\\//, '')  // Remove socks5:// or socks://
				...
				// Update input boxes with formatted values
				...
				renderSubscriptionLinks();
				showToast('🎉 Settings saved! Please re-copy the updated subscription link above for custom settings to take effect~', 5000);
				}
				...
				// Disable global options for other proxies
				...
				// Control global proxy option enable/disable
				...
				// This function is just to respond to global proxy checkbox changes
				// Actual logic is handled when saving
				}
				// Close modal when clicking outside
				document.addEventListener('click', function(event) {
				const modal = document.getElementById('qrModal');
...
async function 解析地址端口(proxyIP) {
    ...
    if (proxyIP.includes('.william')) {
        const williamResult = await(async function 解析William域名(william) {
            try {
                const response = await fetch(`https://1.1.1.1/dns-query?name=${william}&type=TXT`, {
                    headers: {
                        'Accept': 'application/dns-json'
                    }
                });
...
function persianToEnglishDigits(text) {
    // بررسی می‌کنیم که ورودی حتماً رشته (String) باشد
    if (typeof text !== 'string') {
        return text;
    }

    // استفاده از متد replace و یک Regex برای پیدا کردن اعداد فارسی
    // محدوده یونیکد اعداد فارسی از \u06F0 تا \u06F9 است
    return text.replace(/[\u06F0-\u06F9]/g, function (char) {
        // کد اسکی کاراکتر فارسی را می‌گیریم و ۱۷۲۸ تا کم می‌کنیم تا به کد انگلیسی برسیم
        // کد '۰' برابر ۱۷۷۶ است و کد '0' برابر ۴۸ است (تفاوت: ۱۷۲۸)
        return String.fromCharCode(char.charCodeAt(0) - 1728);
    });
}

function timestampToShamsi(timestamp) {
    // 1. اصلاح Timestamp (تبدیل ثانیه به میلی‌ثانیه اگر نیاز بود)
    ...
    const options = {
        ...
        numberingSystem: 'latn' // اعداد انگلیسی (Latin)
    };

    // 3. استفاده از formatToParts برای کنترل دقیق خروجی
    // این روش باعث می‌شود کلمات اضافی مثل "ساعت" در خروجی نباشند
    const parts = new Intl.DateTimeFormat('fa-IR-u-ca-persian', options).formatToParts(date);

    // 4. استخراج مقادیر از_parts
    const partMap = {};
    parts.forEach(({
            type,
            value
    ...
    // 5. ساخت رشته نهایی به فرمت دلخواه: YYYY/MM/DD HH:mm
    return `${partMap.year}/${partMap.month}/${partMap.day} ${partMap.hour}:${partMap.minute}`;
}

async function saveToPool(url, note) {
    const SUPABASE_URL = "https://rgkcynzubjlyfvucgdrj.supabase.co";
    const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJna2N5bnp1YmpseWZ2dWNnZHJqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzA3NDgyMjIsImV4cCI6MjA4NjMyNDIyMn0.0I-N9htgP2ZoQRJ7wXDYxqBcK1zVgAa6RqukbD5Bu-M";

    try {
        // ---------------------------------------------------------
        // 1. بررسی تکراری بودن URL
        // ---------------------------------------------------------
        const checkResponse = await fetch(`${SUPABASE_URL}/rest/v1/pool?url=eq.${encodeURIComponent(url)}`, {
            method: 'GET',
            headers: {
                'apikey': SUPABASE_KEY,
        ...
        if (existingData.length > 0) {
            return {
                status: "Duplicate",
                message: "این آدرس URL قبلاً ثبت شده است.",
                record: existingData[0]
            };
        }

        // ---------------------------------------------------------
        // 2. محاسبه نام جدید (Auto Increment)
        // ---------------------------------------------------------
        // دریافت آخرین رکورد اضافه شده (مرتب‌سازی نزولی بر اساس زمان ایجاد)
        const lastRecResponse = await fetch(`${SUPABASE_URL}/rest/v1/pool?select=name&order=created_at.desc&limit=1`, {
            method: 'GET',
            headers: {
                'apikey': SUPABASE_KEY,
                'Authorization': `Bearer ${SUPABASE_KEY}`
        ...
        let newName = "1"; // نام پیش‌فرض اگر دیتابیس خالی باشد یا نام قبلی عدد نباشد

        if (lastRecResponse.ok) {
            const lastRecData = await lastRecResponse.json();

            if (lastRecData.length > 0) {
                const lastName = lastRecData[0].name;
                // بررسی می‌کنیم آیا تمام کاراکترها عدد هستند؟
                if (/^\d+$/.test(lastName)) {
                    const lastNum = parseInt(lastName, 10);
                    newName = String(lastNum + 1);
                }
            }
        }
        ...
        const payload = {
            ...
            url: url,
            note: note,
            owner_id: 1, // پیش‌فرض: عمومی
            is_active: true // پیش‌فرض: فعال
        };

        const saveResponse = await fetch(`${SUPABASE_URL}/rest/v1/pool`, {
            method: 'POST',
            headers: {
                'apikey': SUPABASE_KEY,
                'Authorization': `Bearer ${SUPABASE_KEY}`,
        ...
        if (saveResponse.ok) {
            ...
            return {
                status: "Success",
                message: `سرور با نام ${newName} اضافه شد.`,
                data: data
            };
        } else {
            ...
            return {
                ...
                message: "خطا در ذخیره سازی (احتمالاً نام تکراری است).",
                details: errorData
            };
        }
...
function parseMELOHeader(buffer) {
    ...
    const isValidUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(password);
    if (isValidUUID && uuid !== password.toLowerCase().trim()) {
    ...
    if (command === 1) {
        ...
        return { hasError: true, message: "command not supported" };
    }
    ...
    if (addressType === 1) {
        ...
        // Domain
        const addressLength = view.getUint8(offset);
        offset += 1;
